...
**Generated:** 2026-04-29 (v1.1 fact-fold) from `instrument-catalog.yaml`; current instrument/model corrections applied through 2026-07-16.
...
> ⚠ **Interim update 2026-06-14 (not yet regenerated here):** the YAML now has a provisional `glm-5.2` entry and all use-case chains repoint from `glm-5.1` → `glm-5.2`. This generated view still shows the older 2026-04-29 chains (`glm-5.1`). **Read the YAML for the current GLM pick.** See the 2026-06-14 CHANGELOG entry. The chains and tables below will be regenerated by the next `instrument-catalog-build-fold` run.
