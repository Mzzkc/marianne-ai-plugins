...
def wrapper(parent_score, parent_workspace, child_workspace, input_dir, out_dir, resources=ROOT):
    ...
    cfg['prompt']['template']=cfg['prompt']['template'].replace("{% set resources = score_dir ~ '/..' %}\n",'').replace("{% set resources = score_dir ~ '/../lab' %}\n",'')
    cfg['prompt']['template']=cfg['prompt']['template'].replace('{{ score_dir }}/../lab',portable(resources)).replace('{{ score_dir }}/..',portable(resources))
    ...
    for items in [cfg['sheet'].get('prelude', []), *cfg['sheet']['cadenzas'].values()]:
        for item in items:
            for key in ('file','directory'):
                if key in item: item[key]=item[key].replace('{{ score_dir }}/../lab',portable(resources)).replace('{{ score_dir }}/..',portable(resources))
