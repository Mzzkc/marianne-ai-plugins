...
class ExpertManifestTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.module = load_script(
            "marianne/skills/marianne-expert/scripts/release_manifest.py",
            "marianne_expert_manifest",
        )
    ...
    def test_manifest_is_relocatable_and_detects_changes(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            (second / "nested" / "data.txt").write_text("changed\n", encoding="utf-8")
            findings = self.module.verify_manifest(second)
            self.assertTrue(any("nested/data.txt" in finding for finding in findings))
    ...
    def test_manifest_detects_unlisted_extra_file(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertTrue(any("extra.txt" in item for item in self.module.verify_manifest(root)))
